...
var _X_GTEXTSIZE = 16;
var _X_GBACKGRNDCOLOUR = [50, 50, 55];
var _X_GTEXTOFFSETX = 0;
var _X_GTEXTOFFSETY = 0;
var _X_GAUTORESZ = false;
...
var _X_GLOBALVARS = [["test",90132],["sys-info","Virtual Terminal (CXJSTERM) running in JavaScript with p5.js, \nOriginal screen start setting: " + _X_GWIDTH + "x" + _X_GHEIGHT + "@TargetFPS" + _X_GTARGETFPS]];
var _X_GALIASES = [["test", "var", ":alert(\"received \" + \"%var%\");"],["test2",":console.log(\"test\")"]];
...
function _X_FINALCMD(_X_TFCMD){
	...
	if(_X_TF!=null){
		...
		for(_X_Ti=0;_X_Ti<_X_GALIASES[_X_TF][_X_GALIASES[_X_TF].length-1].length;_X_Ti++){
			if(_X_GALIASES[_X_TF][_X_GALIASES[_X_TF].length-1].charAt(_X_Ti) === "%"){
				_X_TE = !_X_TE;
				if(_X_TE){
					_X_TSTART = _X_Ti;
					_X_TVAR = "";
				...
				//_X_Ti++;
				if(_X_TE){
					_X_TVAR+=_X_GALIASES[_X_TF][_X_GALIASES[_X_TF].length-1].charAt(_X_Ti);
				}
			}
		}
